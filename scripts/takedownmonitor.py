...
def ebaytakedowns():
    ...
    successful_takedowns_ebay_bulk = ebayads[~(ebayads['url'].isin(ebayAPI['url']))].reset_index(drop=True)
    
    #sending these to additionalCheckEbay function to re-run the search using the product title
    additionnal_check_data=additionalCheckEbay(successful_takedowns_ebay_bulk)
    
    #compare new ads with the initial list
    successful_takedowns_ebay = successful_takedowns_ebay_bulk[~(successful_takedowns_ebay_bulk['url'].isin(additionnal_check_data['url']))].reset_index(drop=True)
    return successful_takedowns_ebay, ebayAPI
...
ebay_takedowns, ebayAPI,  = ebaytakedowns()
ml_takedowns, mlAPI,  = mltakedowns()
...
combined_takedowns=oneframe(ebay_takedowns, ml_takedowns)
combined_takedowns.to_csv(r'C:\Splunk\takedowns.csv')
combined_takedowns=combined_takedowns.drop(columns=['product', 'country'])
if combined_takedowns.empty:
    logger.info('No taken down ads found')
    print('No takedowns found')
    
else:
    combined_takedowns["takedown_confirmed"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    combined_takedowns["review"] = 'Successful Takedown'
    combined_takedowns.set_index("advert_id", inplace=True)

    combined_takedowns.to_sql(
        "takedowns",
        con=sqlengine,
        if_exists="append"
    )
    ...
    for id_advert in combined_takedowns.index:
        print(id_advert)
        update_sql="""Update hades.advert SET review='Successful Takedown' WHERE advert_id=%s"""
    ...
    logger.info('Takedowns updated successfully, number of ads:%s' % numberofads)
    print('Takedowns updated successfully, number of ads:', numberofads)
